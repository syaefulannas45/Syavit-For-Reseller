      ...
      <section className="lg:px-[75px] px-[10px] lg:flex justify-between items-center">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            <p className="text-tittle font-bold lg:text-[36px] text-[25px]">SYAZAH</p>
          </div>
          <div onClick={() => setToggle(!toggle)} className="cursor-pointer mr-3">
            <i className={`fa-solid fa-bars text-3xl text-text lg:hidden ${!toggle ? "block" : "hidden"}`}></i>
            <i className={`fa-solid fa-xmark text-3xl text-text lg:hidden ${toggle ? "block" : "hidden"}`}></i>
          </div>
        </div>
        <nav>
          <ul
            className={`lg:space-x-5 ${toggle ? "block" : "hidden"} ${
